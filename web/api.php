<?php
    ...
    function get_profiles($mode) {
        global $libernet_dir;
        $profiles = array();
        if ($handle = opendir($libernet_dir.'/config/'.$mode.'/')) {
    ...
    function get_config($mode, $profile) {
        ...
        $config = null;
        if ($profile) {
            $config = file_get_contents($libernet_dir.'/config/'.$mode.'/'.$profile.'.json');
        } else {
            $system_config = file_get_contents($libernet_dir.'/system/config.json');
            $system_config = json_decode($system_config);
            $config = file_get_contents($libernet_dir.'/config/'.$mode.'/'.$system_config->tunnel->profile->$mode.'.json');
        }
        ...
        switch ($json['action']) {
            ...
            case 'save_config':
                if (isset($json['data'])) {
                    ...
                    switch ($mode) {
                        // ssh
                        case 0:
                            file_put_contents($libernet_dir.'/config/ssh/'.$profile.'.json', json_encode($config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                        ...
                        case 1:
                            ...
                            switch ($protocol) {
                                // vmess
                                case "vmess":
                                    $vmess_config = file_get_contents($libernet_dir.'/config/v2ray/templates/vmess.json');
                                    ...
                                    file_put_contents($libernet_dir.'/config/v2ray/'.$profile.'.json', json_encode($vmess_config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                                    json_response('V2Ray vmess config saved');
                                    break;
                                // vless
                                case "vless":
                                    $vless_config = file_get_contents($libernet_dir.'/config/v2ray/templates/vless.json');
                                    ...
                                    file_put_contents($libernet_dir.'/config/v2ray/'.$profile.'.json', json_encode($vless_config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                                    json_response('V2Ray vless config saved');
                                    break;
                                // trojan
                                case "trojan":
                                    $trojan_config = file_get_contents($libernet_dir.'/config/v2ray/templates/trojan.json');
                                    ...
                                    file_put_contents($libernet_dir.'/config/v2ray/'.$profile.'.json', json_encode($trojan_config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                        ...
                        // ssh-ssl
                        case 2:
                            file_put_contents($libernet_dir.'/config/ssh_ssl/'.$profile.'.json', json_encode($config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                        ...
                        case 4:
                            ...
                            switch ($plugin) {
                                case 'obfs-local':
                                    $shadowsocks_config = file_get_contents($libernet_dir.'/config/shadowsocks/templates/simple-obfs.json');
                                    ...
                                    break;
                                case 'ck-client':
                                    $shadowsocks_config = file_get_contents($libernet_dir.'/config/shadowsocks/templates/cloak.json');
                                    $shadowsocks_config = json_decode($shadowsocks_config);
                                    // set cloak uid
                                    ...
                                    break;
                                default:
                                    $shadowsocks_config = file_get_contents($libernet_dir.'/config/shadowsocks/templates/normal.json');
                                    $shadowsocks_config = json_decode($shadowsocks_config);
                                    break;
                            }
                            ...
                            file_put_contents($libernet_dir.'/config/shadowsocks/'.$profile.'.json', json_encode($shadowsocks_config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                            json_response('Shadowsocks config saved');
                            break;
                        // openvpn
                        case 5:
                            file_put_contents($libernet_dir.'/config/openvpn/'.$profile.'.json', json_encode($config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                            json_response('OpenVPN config saved');
                            break;
                        // ssh-ws-cdn
                        case 6:
                            file_put_contents($libernet_dir.'/config/ssh_ws_cdn/'.$profile.'.json', json_encode($config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
            ...
            case 'apply_config':
                if (isset($json['data'])) {
                    ...
                    switch ($mode) {
                        // ssh
                        case 0:
                            $ssh_config = file_get_contents($libernet_dir.'/config/ssh/'.$profile.'.json');
                        ...
                        // v2ray
                        case 1:
                            $v2ray_config = file_get_contents($libernet_dir.'/config/v2ray/'.$profile.'.json');
                        ...
                        // ssh-ssl
                        case 2:
                            $sshl_config = file_get_contents($libernet_dir.'/config/ssh_ssl/'.$profile.'.json');
                        ...
                        // trojan
                        case 3:
                            $trojan_config = file_get_contents($libernet_dir.'/config/trojan/'.$profile.'.json');
                            $trojan_config = json_decode($trojan_config);
                        ...
                        // shadowsocks
                        case 4:
                            $shadowsocks_config = file_get_contents($libernet_dir.'/config/shadowsocks/'.$profile.'.json');
                            $shadowsocks_config = json_decode($shadowsocks_config);
                        ...
                        // openvpn
                        case 5:
                            $openvpn_config = file_get_contents($libernet_dir.'/config/openvpn/'.$profile.'.json');
                        ...
                        // ssh-ws-cdn
                        case 6:
                            $ssh_ws_cdn_config = file_get_contents($libernet_dir.'/config/ssh_ws_cdn/'.$profile.'.json');
            ...
            case 'delete_config':
                if (isset($json['data'])) {
                    ...
                    switch ($mode) {
                        case 0:
                            unlink($libernet_dir.'/config/ssh/'.$profile.'.json');
                            json_response('SSH config removed');
                            break;
                        case 1:
                            unlink($libernet_dir.'/config/v2ray/'.$profile.'.json');
                            json_response('V2Ray config removed');
                            break;
                        case 2:
                            unlink($libernet_dir.'/config/ssh_ssl/'.$profile.'.json');
                            json_response('SSH-SSL config removed');
                            break;
                        case 3:
                            unlink($libernet_dir.'/config/trojan/'.$profile.'.json');
                            json_response('Trojan config removed');
                            break;
                        case 4:
                            unlink($libernet_dir.'/config/shadowsocks/'.$profile.'.json');
                            json_response('Shadowsocks config removed');
                            break;
                        case 5:
                            unlink($libernet_dir.'/config/openvpn/'.$profile.'.json');
                            json_response('OpenVPN config removed');
                            break;
                        case 6;
                            unlink($libernet_dir.'/config/ssh_ws_cdn/'.$profile.'.json');
