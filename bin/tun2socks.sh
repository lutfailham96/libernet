...
start_tun2socks() {
  ...
  if [[ $TUN2SOCKS_MODE == "false" ]]; then
    ...
    # using another go-tun2socks
    #screen -AmdS go-tun2socks go-tun2socks -loglevel silent -device "${TUN_DEV}" -proxy socks5://"${SOCKS_SERVER}" -tcp-auto-tuning
  else
    ...
    # using another tun2socks
    #screen -AmdS tun2socks tun2socks -loglevel silent -device "${TUN_DEV}" -proxy socks5://"${SOCKS_SERVER}"
  fi
  # removing default route
  echo "${DEFAULT_ROUTE}" > "${ROUTE_LOG}" \
    && ip route del "${DEFAULT_ROUTE}"
  # add default route to tun2socks
  ip route add default via "${TUN_ADDRESS}" metric ${DEFAULT_METRIC}
...
stop_tun2socks() {
  ...
  if [[ $TUN2SOCKS_MODE == "false" ]]; then
    ...
    # using another tun2socks
    #kill "$(screen -list | grep tun2socks | awk -F '[.]' '{print $1}')"
  fi
  # recover default route
  ip route add "$(cat "${ROUTE_LOG}")" \
    && rm -rf "${ROUTE_LOG}"
  # remove default route to tun2socks
  ip route del default via "${TUN_ADDRESS}" metric ${DEFAULT_METRIC}
...
route_add_ip() {
  ...
  ip route add "${SERVER_IP}" via "${GATEWAY}" metric ${DEFAULT_METRIC} &
  ip route add "${CDN_IP}" via "${GATEWAY}" metric ${DEFAULT_METRIC} &
  for IP in "${PROXY_IPS[@]}"; do
    ip route add "${IP}" via "${GATEWAY}" metric ${DEFAULT_METRIC} &
  done
